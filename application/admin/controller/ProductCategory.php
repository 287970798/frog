...
use app\common\model\ProductMultilingualCategory;
use think\Db;
...
class ProductCategory extends BaseController
{
    ...
    {
        $cats = \app\common\model\ProductCategory::all();
    ...
    {
        $cats = (new \app\common\model\ProductCategory())->field('id,pid,name')->select();
    ...
    public function add()
    {
        ...
        if ($this->request->isAjax() && $this->request->post()) {
            ...
            Db::startTrans();
            try {
                $catModel = new \app\common\model\ProductCategory();
                ...
                if ($multilingual === 1) {
                    ...
                    $MultiModel = new ProductMultilingualCategory();
...
//                        $multi_data[] = ['language_id' => $language_id, 'category_id' => $id, 'name' => $language_name];
                        // 查询是否已存在
                        $one = ProductMultilingualCategory::where('language_id', $language_id)->where('category_id', $id)->find();
                        if ($one) {
                            // 存在则更新
                            ...
                            // 不存在则新增
                            $multiCateModel = new ProductMultilingualCategory();
                            $multiCateModel->save(
...
//                    $result2 = $MultiModel->saveAll($multi_data);
        ...
        if ($multilingual === 1) {
            $cat = \app\common\model\ProductCategory::get($id);
    ...
    public function delete()
    {
        ...
        $id = $this->request->param('id');
        $hasChildren = \app\common\model\ProductCategory::where('pid', $id)->count();
        if ($hasChildren > 0) {
            return mkRestful(1, '下有子栏目，不能删除');
        }
        $result = \app\common\model\ProductCategory::destroy($id);
        if ($result === 1) {
